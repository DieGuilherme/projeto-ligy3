# FATURAMENTO LIGY >>> DIRETAMENTE NO STREAMLIT PARA SER DE FÁCIL UTILIZAÇÃO
...
if uploaded_file:
    ...
    df_temp["economia_percebida"] = np.where(df_temp["s_ligy"] == 0, 0, df_temp["economia_real"] / df_temp["s_ligy"])
    ...
    st.dataframe(df_temp[[
        "cliente_ref", "Valor_Ligy", "consumo (kWh)", "tipo_forn", "custo_disp",
        "limite_comp", "limite2", "cred_a_acumular", "cons_faturado", "val_add", "val_cons_final"
    ]])

    st.subheader("🔹 Tabela Fatura Ligy")
    st.dataframe(df_temp[[
        "cliente_ref", "sub_energia_s_gd", "fat_enel_s_gd", "sub_energia_gd", "dif_cons_injec", "tx_ip ($$)", 
        "cob_des_add", "val_cons_final", "fatura_enel_real", "benef_gd", "benef_ligy", "s_ligy", "c_ligy", 
        "economia_real", "economia_percebida", "fatura_ligy", "carbono", "farol"
    ]])
