...
if uploaded_file:
    ...
    df_temp["ref.mes"] = pd.to_datetime(df_temp["ref_fat_cli"]).dt.strftime("%m")
    ...
    df_temp["economia_percebida"] = np.where(np.where(df_temp["s_ligy"] == 0, 0, df_temp["economia_real"]/df_temp["s_ligy"]) == nan, 0, round(np.where(df_temp["s_ligy"] == 0, 0, df_temp["economia_real"]/df_temp["s_ligy"]),2)) + '%'
